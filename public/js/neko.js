...
function gE(e, i) {
    if (l) {
        const o = (i = i || self).document.layers;
        if (o[e]) return o[e];
        for (let s = 0; s < o.length;) t = gE(e, o[s++]);
        return t
    }
...
const checkerboardEccentricity = 10;
const nDelayVariance = 20;
const nFirstRealFrame = 5;

function Neko(x, y, active, imagedirectory) {
    this.findHome = function () {
        this.homeX = eval(this.homeXfn), this.homeY = eval(this.homeYfn)
    }, x || (x = 0), y || (y = 0), parseInt(x) != x ? (this.homeXfn = x, this.homeYfn = y, this.findHome(), x = this.homeX, y = this.homeY) : (x = parseInt(x), y = parseInt(y)), active || (active = !1), !imagedirectory && window.NekoType && (imagedirectory = window.NekoType), imagedirectory || (imagedirectory = "i"), window.remoteimages || (imagedirectory = "/" + imagedirectory), this.directory = imagedirectory;
    const aPreLoad = ["alert", "still", "nrun1", "nrun2", "nerun1", "nerun2", "erun1", "erun2", "serun1", "serun2", "srun1", "srun2", "swrun1", "swrun2", "wrun1", "wrun2", "nwrun1", "nwrun2", "yawn", "sleep1", "sleep2", "itch1", "itch2", "nscratch1", "nscratch2", "escratch1", "escratch2", "sscratch1", "sscratch2", "wscratch1", "wscratch2"];
    this.aGifs = [];
    for (let i = 0; i < aPreLoad.length; i++) {
        const imgTemp = new Image;
        imgTemp.src = this.directory + "/" + aPreLoad[i] + ".gif", this.aGifs[aPreLoad[i]] = imgTemp
    }
    this.whichNeko = aNekos.length, aNekos[aNekos.length] = this, this.a_resting = ["", 'this.SetBehavior("wakingup")', "this.chooseIdle()", "8", "1", "still"], this.a_itching = ["", 'this.SetBehavior("wakingup")', 'this.SetBehavior("resting")', "6", ".5", "itch2", "itch1"], this.a_scratching = ["", 'this.SetBehavior("wakingup")', 'this.SetBehavior("resting")', "4", "2", "scratch1", "scratch2"], this.a_yawning = ["", 'this.SetBehavior("wakingup")', 'this.SetBehavior("resting2")', "5", "1", "yawn"], this.a_resting2 = ["", 'this.SetBehavior("wakingup")', 'this.SetBehavior("sleeping")', "9", "1", "still"], this.a_wakingup = ['this.SetBehavior("resting")', "", 'this.SetBehavior("chasing")', "1", "1", "alert", "still"], this.a_chasing = ['this.SetBehavior("resting")', "", "", "0", "1", "run1", "run2"], this.a_sleeping = ["", 'this.SetBehavior("wakingup")', "", "0", "1", "sleep1", "sleep1", "sleep1", "sleep2", "sleep2", "sleep2"], this.behaviorRepetition = 0, this.loopTimes = 0;
    const strLayer = "layerNeko" + this.whichNeko, strImage = "imageNeko" + this.whichNeko,
        strImageSrc = this.directory + "/still.gif", strNekoObj = "aNekos[" + this.whichNeko + "]",
        strContent = "<a ondblclick='document.location.href=\"https://webneko.net\"' href='https://webneko.net' onmouseover='" + strNekoObj + ".message();return true' onmouseout='window.status = \"\"' onclick='" + strNekoObj + ".active = !" + strNekoObj + ".active;return false;' onfocus='this.blur()'><img border='0' name='" + strImage + "' src='" + strImageSrc + "'></a>";
...
const sNekoMessage = "Click Neko and he'll chase your mouse!    Double-click to visit Neko's home (https://webneko.net)";
const sNekoMessageCaught = sNekoMessage;

function NekoMessage() {
    window.status = this.caught ? sNekoMessageCaught : sNekoMessage
}

function NekoTargetMouse() {
    const e = mouse.x + 4, t = mouse.y - 20;
...
function NekoUpdateImage() {
    let strImage;
    let paImages = eval("aNekos[" + this.whichNeko + "].a_" + this.behavior);
    if (this.frame >= paImages.length) if (this.behaviorRepetition++, 0 != this.loopTimes && this.behaviorRepetition >= this.loopTimes) {
        ...
        paImages = eval("aNekos[" + this.whichNeko + "].a_" + this.behavior);
    } else this.frame = nFirstRealFrame;
    if (this.aGifs[this.direction + paImages[this.frame]]) {
        strImage = this.aGifs[this.direction + paImages[this.frame]].src;
        this.image.src = strImage
    } else if (this.aGifs[paImages[this.frame]]) {
        strImage = this.aGifs[paImages[this.frame]].src;
        this.image.src = strImage
    } else if (this.looseDirection + this.aGifs[paImages[this.frame]]) {
        strImage = this.aGifs[this.looseDirection + paImages[this.frame]].src;
        this.image.src = strImage
...
function NekoFindDirection() {
    if (0 != t || 0 != e) {
        var e = -1 * this.dy, t = this.dx, i = Math.abs(t), o = "", s = "", a = Math.abs(e) / i, r = a < .41421,
            h = a > 2.4142;
...
function nekoChooseIdle() {
    const e = ["resting", "yawning", "itching", "scratching"], t = Math.floor(Math.random() * e.length);
...
function startANeko() {
    ...
    window.startNekoX && (e = window.startNekoX), window.startNekoY && (t = window.startNekoY), parseInt(e) != e && (window.onresize = function () {
        for (let e = 0; e < aNekos.length; e++) aNekos[e].homeXfn && aNekos[e].findHome()
    });
    new Neko(e, t, !1);
...
function boxBoundHeight(e) {
    0 == e || e || (e = this.height());
    let t = this.height() - 12;
    return e < 20 && (e = 20), e > t && (e = t), e
}

const checkerboardScale = 20;

let checkerboard = new Array(checkerboardScale);
let i = 0;

function boxSetBoard(e, t, i) {
    let o = Math.floor(t / this.width() * checkerboardScale), s = Math.floor(i / this.height() * checkerboardScale);
...
Neko.prototype.message = NekoMessage, Neko.prototype.TargetMouse = NekoTargetMouse, Neko.prototype.Show = NekoShow, Neko.prototype.Hide = NekoHide, Neko.prototype.TargetHome = NekoTargetHome, Neko.prototype.CalculateDistance = NekoCalculateDistance, Neko.prototype.SetBehavior = NekoSetBehavior, Neko.prototype.UpdateImage = NekoUpdateImage, Neko.prototype.MoveAStep = NekoMoveAStep, Neko.prototype.FindDirection = NekoFindDirection, Neko.prototype.Think = NekoThink, Neko.prototype.chooseIdle = nekoChooseIdle, window.onloadOriginal = new Function, window.onload && (window.onloadOriginal = window.onload), window.NekoNoDefault || (window.onload = startANeko), mouse = new mouse, box.prototype.boundWidth = boxBoundWidth, box.prototype.boundHeight = boxBoundHeight, box.prototype.setBoard = boxSetBoard, box.prototype.checkBoard = boxCheckBoard, box = new box, document.onmousemove = function (e) {
    const t = e ? e.pageX : event.x + document.body.scrollLeft, i = e ? e.pageY : event.y + document.body.scrollTop;
    mouse.x = box.boundWidth(t), mouse.y = box.boundHeight(i)
}, document.captureEvents && document.captureEvents(Event.MOUSEMOVE);
for (; i < checkerboardScale; i++) {
    checkerboard[i] = new Array(checkerboardScale);
    for (let j = 0; j < checkerboardScale; j++) checkerboard[i][j] = 0
}
